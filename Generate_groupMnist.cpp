...
char tempstr1[M];
char tempstr2[M];
char  cc[256][10] = { { "00000000" }, { "00000001"}, { "00000010" }, { "00000011" }, {"00000100" }, { "00000101" }, { "00100111" }, { "00101000" }, { "00101001" }, { "00101010" }, { "00101011" }
...
void readfile(char arry[]){
	...
	*/
	...
	for (int i = 0; i < 6273; i++){
		...
		temp[count1] = arry[i]- '0';
		if (i % 8 == 0)		final[count2++]=E(i);
		}
	count1++;
	ofstream   outfile;
	outfile.open("G:\\python\\CC_out.txt",ios::app);
	for (int i = 0; i < 28; i++){
		for (int j = 0; j < 28; j++){
			outfile << final[28*i+j] << " ";
		}
		outfile << endl;
	}
	outfile.close();
...
int main(){
	...
	for (int i = 0; i < sizeof(NDb) / sizeof(NDb[0]); i++){
