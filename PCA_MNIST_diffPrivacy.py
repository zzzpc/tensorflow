...
train = Dataset(x_train_reduction, one_hot_train)

test = Dataset(x_test_reduction, one_hot_test)
...
if __name__ == "__main__":
    ...
    for step  in range(epoch):
        ...
        for i in range(600):
            ...
            if i % 100 == 0:
                print("train accuracy :",accuracy.eval({x: batch_xs, y_: batch_ys, keep_prob: 1.0}),"当前学习率为%f:" % ra)
        # 计算准确率
        test_acc=accuracy.eval({x:x_test_reduction, y_: mnist.test.labels, keep_prob: 1.0},session=sess)
        g=sess.run(add_global)
        print("第%d个epoch训练完毕 测试精确度未%.2f ，马上进入第%d个epoch的训练" %(step,test_acc,g) )
