...
if __name__ == '__main__':
    ...
    test_images = load_test_images('MNIST_data/t10k-images.idx3-ubyte')
    test_labels = load_test_labels('MNIST_data/t10k-labels.idx1-ubyte')

    # 查看前十个数据及其标签以读取是否正确
    time1 = time.time()

    f = open('zpc.txt', 'r+', encoding='utf-8')
    for i in range(len(test_images)):
        tt = test_images[i].tolist()
        for j in range(28):
            for k in range(28):
                cc=int(tt[j][k])
                if(i==1):
