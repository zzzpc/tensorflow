...
{   
	...
	if (l < q[7]) return 8; 
	else if (l < q[7] + q[6]) return 7;
	else if (l < q[7] + q[6] + q[5])return 6;
	else if (l < q[7] + q[6] + q[5] + q[4])return 5;
	else if (l < q[7] + q[6] + q[5] + q[4] + q[3])return 4;
	else if (l < q[7] + q[6] + q[5] + q[4] + q[3] + q[2])return 3;
	else if (l < q[7] + q[6] + q[5] + q[4] + q[3] + q[2] + q[1])return 2;
	else return 1;
...
void f(char s[])		//生成s[]的负数据库	
{
	...
	do
	{
		...
		else if (t<p[1] + p[2])
		{
			...
			attr = rand1(m / 8);
			v.p[0] = diff1 + attr * 8-1;      //生成的反转位
			v.c[0] = '1' + '0' - s[v.p[0]];
			while (diff2 == diff1){
				diff2 = generateRandomNumbers(rand1());
			}

			v.p[1] = diff2 + attr * 8-1;
			v.c[1] = '1' + '0' - s[v.p[1]];
