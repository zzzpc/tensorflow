...
class CifarDate:
    ...
    def next_batch(self, batch_size):
        ...
        self.start = end
        return batch_data, batch_labels
train_filename = ['/input/cifar10/qk_1.txt', '/input/cifar10/qk_2.txt', '/input/cifar10/qk_3.txt',
                  '/input/cifar10/qk_4.txt', '/input/cifar10/qk_5.txt']
test_filename = ['/input/cifar10/qk_test.txt']
...
if __name__ == '__main__':
    ...
    result_x_images_train=data_aug_train()


    # 训练
    output = vgg_net(result_x_images_train, weights, biases, keep_prob)
    ...
    output_test = vgg_net(result_x_images_test, weights, biases, keep_prob)
    correct_prediction_test = tf.equal(tf.argmax(output, 1), tf.argmax(y_, 1))
        ...
        for ep in range(epochs):
            ...
            for i in range(steps):
                ...
                _, batch_loss = sess.run([train_step, cross_entropy],
                                         feed_dict={x: batch_x, y_: batch_y, keep_prob: 0.5, train_flag: True,
                                                    learning_rate: alpha})
                # print(batch_x[0])
                batch_acc = accuracy.eval(
                    feed_dict={x: batch_x, y_: batch_y, keep_prob: 1.0, train_flag: True, learning_rate: alpha})
            ...
            for i in range(156):
                ...
                acc_test= sess.run([accuracy_test],feed_dict={x: batch_x, y_: batch_y, keep_prob: 1.0, train_flag: False})
